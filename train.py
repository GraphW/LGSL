...
def parse():
    ...
    parser.add_argument('--lr-decay', type=int, default=200, help='After how many epochs to decay LR by a factor of '
                                                                  'gamma.')
    parser.add_argument('--weight-decay', type=float, default=0.04, help='L2 penalty of model')
    ...
    # GAT层的相关参数
    parser.add_argument('--encoder-hidden', type=int, nargs='+', default=32, help='Number of hidden units in encoder.')
    parser.add_argument('--reserved-hidden', type=int, nargs='+', default=32, help='Number of hidden units in reserved '
                                                                                   'time series encoder.')
...
def train(epoch, best_val_loss, best_val_acc, args, params):
    ...
    # node_coordinate = params['node_coordinate']
    ...
    for index, (data, labels) in enumerate(train_iterable):
        ...
        # print(labels)
        labels = F.one_hot(labels, num_classes=4).float()  # 将labels转成one-hot
        data = data.to(device)
        labels = labels.to(device)
        optimizer.zero_grad()
        # 这里是输出

        output = model(data, node_class, receive, send, args.temp)  # 输出边的连接概率，是一种什么样的概率呢？注意力系数，节点的表征？
        loss_ent = label_cross_entropy(output, labels)
        label_acc, _ = label_accuracy(output, labels)
        loss = loss_ent  # 将交叉熵作为损失
...
def test(args, params):
    ...
    model.eval()
    # model_file='logs/2022-03-19T14-28-50.264972/GAT_encoder.pt'
    ...
    for index, (data, labels) in enumerate(tqdm(test_loader)):
        ...
        # print(labels)
        # _, real = labels.max(-1)
        # real = real.cpu().tolist()
        # real_label = real_label + real
    ...
    # print ('same_count={}'.format(same_count))
